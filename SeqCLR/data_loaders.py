# Data loaders for SeqCLR from zac implementation.
...
class ContrastiveDataset(data.Dataset):
    ...
    def init_params_from_scratch(self, path, total_points, window_size, sfreq, bw,
        ...
        self.SFREQ = sfreq
        # band width (?) see https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.iirnotch.html
        self.BW = bw
        ...
        self.pairs, self.labels = self.get_samples_and_labels(
            size=total_points, window_size=window_size)
        pass
    ...
    def get_samples_and_labels(self, size, window_size):
        ...
        """
        pairs = [[None, None]
                 for _ in range(size)]  # np.zeros((size, 2), dtype=int)
        labels = None
    ...
    def get_augmentation_set(self):
        ...
        for j in range(self.NUM_CHANNELS):
            ...
            selected_augmentations = random.sample(
                list(self.available_augmentations.keys()), self.NUM_AUGMENTATIONS)
            ...
            for _, curr_augmentation in enumerate(selected_augmentations):
                ...
                # augmentation that requires float val
                if curr_augmentation in ['amplitude_scale', 'DC_shift', 'additive_Gaussian_noise', 'band-stop_filter']:
                    # see https://stackoverflow.com/questions/6088077/how-to-get-a-random-number-between-a-float-range
                    curr_augmentation_val = random.uniform(
                        self.available_augmentations[curr_augmentation][0], self.available_augmentations[curr_augmentation][1])

                # augmentation that requires int val
                elif curr_augmentation in ['time_shift', 'zero-masking']:
                    # see https://stackoverflow.com/questions/3996904/generate-random-integers-between-0-and-9
                    curr_augmentation_val = random.randint(
                        self.available_augmentations[curr_augmentation][0], self.available_augmentations[curr_augmentation][1])
                    if curr_augmentation == 'zero-masking':
                        curr_augmentation_val = [
                            curr_augmentation_val,
                            random.randint(0, self.TEMPORAL_LEN-1)]

                else:
                    raise NotImplementedError(
                        "curr_augmentation == "+str(curr_augmentation)+" not recognized for value sampling")
        ...
        for j, curr_augmentation_set in enumerate(augmentations):
            ...
            for _, curr_augmentation in enumerate(list(curr_augmentation_set.keys())):
                ...
                if curr_augmentation == 'amplitude_scale':
                    ...
                    raise NotImplementedError(
                        "curr_augmentation == "+str(curr_augmentation)+" not recognized for application")
