...
if __name__ == "__main__":
    ...
    if CACHE_WINDOWS == False:
        ...
        ids_to_load = window_and_split(tuh_preproc, save_dir=save_dir_2 , overwrite=True, window_size_samples=15000,n_jobs=2, save_dir_index=save_dir_indexes)
    else:
        with open(save_dir_indexes, 'rb') as f:
            ids_to_load = pickle.load(f)

    windowed_datasets = load_concat_dataset(os.path.join(save_dir_2, 'fif_ds'), preload=False,ids_to_load=ids_to_load)
