...
class SQDataset(Dataset):  # type: ignore
...
# SeqCLR (SQ) CODE BLOCK #####################################################################################
# SACL (SA) CODE BLOCK #######################################################################################
class SADataset(Dataset): # NOTE THAT IF perturb_orig_signal==False AND individual_id is None AND num_individuals_in_label is None, THEN THIS DATASET *SHOULD* BE EQUIVALENT TO SQDataset
    def __init__(self, cached_ps_dataset, path=None, total_points=None, window_size=None, sfreq=1000, bw=5, 
    ...
    def get_samples_and_labels(self, size, window_size):
        ...
        for i in range(size):
            ...
            if not self.randomized_augmentation: # decide whether or not to save augmentation strategy for curr sample
                if self.perturb_orig_signal:
                    second_val[0] = self.get_augmentation_set()  # type: ignore
                second_val[1] = self.get_augmentation_set()  # type: ignore
            # else:
            #     second_val = [None, None]
...
def load_SSL_Dataset(task_id, cached_datasets_list_dir=None, total_points_val=2000, tpos_val=30, tneg_val=120, 
    ...
    combined_dataset = torch.utils.data.ConcatDataset(datasets_list)  # type: ignore
    ...
    train_set, val_set, test_set = torch.utils.data.random_split(combined_dataset,   # type: ignore
...
def load_SSL_Dataset_Based_On_Subjects(task_id, cached_datasets_list_dir=None, total_points_val=2000, tpos_val=30, tneg_val=120, 
    ...
    datasets_list = datasets_list[shuffle_inds]  # type: ignore
    ...
    train_set = torch.utils.data.ConcatDataset(datasets_list[:num_train_subjects])  # type: ignore
    train_set = torch.utils.data.random_split(train_set,   # type: ignore
                                              [len(train_set)], 
                                              generator=torch.Generator().manual_seed(random_seed)
    )
    val_set = torch.utils.data.ConcatDataset(datasets_list[num_train_subjects:num_train_subjects+num_val_subjects])  # type: ignore
    val_set = torch.utils.data.random_split(val_set,   # type: ignore
                                            [len(val_set)], 
                                            generator=torch.Generator().manual_seed(random_seed)
    )
    test_set = torch.utils.data.ConcatDataset(datasets_list[num_train_subjects+num_val_subjects:])  # type: ignore
    test_set = torch.utils.data.random_split(test_set,   # type: ignore
                                             [len(test_set)], 
                                             generator=torch.Generator().manual_seed(random_seed)
    )
...
def load_Downstream_Dataset(cached_datasets_list_dir=None, n_cross_val_splits=5,  
    ...
    for train_index, test_index in cross_val_kfold.split(x_vals, y_vals, groups):
        ...
        train_set, val_set = torch.utils.data.random_split(orig_train_set, # type: ignore , see https://pytorch.org/docs/stable/data.html
